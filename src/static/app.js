      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = details.participants.length
          ? details.participants
              .map(
                (participant) =>
                  `<li><span class="participant-name">${participant}</span><button class="remove-participant-btn" data-activity="${encodeURIComponent(
                    name
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        signupForm.reset();
        await fetchActivities(); // reload the activity cards immediately
      } else {
